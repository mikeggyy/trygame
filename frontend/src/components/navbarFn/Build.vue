...
const initData = () => {
  ...
  if (config().totalAssets < money.value) {
...
<template>
  <div class="Build">
    <div v-if="isWarning == true">
      <DescriptionForPopup :title="`${popupState().buildType}`" :content="`可用金主不足`" @cancel="notEnoughMoneyTips" />
    </div>
    <div v-else>
      <!-- 介紹人來推薦 -->
      <div v-if="introducerCheck == true">
        <TalkForPopup :peopleItem="config().introducerItem" :talking="talking().getIntroducerHelpHireTalking('木工')" />
